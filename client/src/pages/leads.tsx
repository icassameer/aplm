...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  ...
  CONTACTED: { label: "Contacted", color: "bg-chart-2 text-white dark:text-white", icon: Phone },
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {user?.role === "TEAM_LEADER" && (
            <Select value={assignmentFilter} onValueChange={(v) => { setAssignmentFilter(v); setPage(1); }}>
              <SelectTrigger className="w-44" data-testid="select-assignment-filter">
      ...
      {isAgencyAdmin ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {Object.entries(statusConfig).map(([key, config]) => {
            ...
            const count = leadStatsData?.[key] || 0;
            return (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {lead.remarks && <p className="text-xs text-muted-foreground mt-1 truncate max-w-md">{lead.remarks}</p>}
                        </div>
                      </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <form onSubmit={(e) => { e.preventDefault(); updateMutation.mutate(); }} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Remarks</Label>
              <Textarea data-testid="input-edit-remarks" value={editForm.remarks} onChange={(e) => setEditForm({ ...editForm, remarks: e.target.value })} rows={3} />
            </div>
